...
exports.FhevmContractsRepository = void 0;
...
const ZamaFheDecryptionOracle_js_1 = require("./ZamaFheDecryptionOracle.js");
class FhevmContractsRepository {
    ...
    static async create(ethersReadonlyProvider, config) {
        if (!ethers_1.ethers.isAddress(config.aclContractAddress)) {
            throw new error_js_1.FhevmError(`Invalid ACL contract address ${config.aclContractAddress}`);
        }
        if (!ethers_1.ethers.isAddress(config.kmsContractAddress)) {
            throw new error_js_1.FhevmError(`Invalid KMSVerifier contract address ${config.kmsContractAddress}`);
        }
        if (config.zamaFheDecryptionOracleAddress !== undefined) {
            if (!ethers_1.ethers.isAddress(config.zamaFheDecryptionOracleAddress)) {
                throw new error_js_1.FhevmError(`Invalid DecryptionOracle contract address ${config.zamaFheDecryptionOracleAddress}`);
            }
        }
        const repo = new FhevmContractsRepository();
        __classPrivateFieldSet(repo, _FhevmContractsRepository_acl, await ACL_js_1.ACL.create(ethersReadonlyProvider, config.aclContractAddress, config.aclAbi, config.aclProperties), "f");
        __classPrivateFieldSet(repo, _FhevmContractsRepository_fhevmExecutor, await FHEVMExecutor_js_1.FHEVMExecutor.create(ethersReadonlyProvider, __classPrivateFieldGet(repo, _FhevmContractsRepository_acl, "f").fhevmExecutorAddress, config.fhevmExecutorAbi, config.fhevmExecutorProperties), "f");
        __classPrivateFieldSet(repo, _FhevmContractsRepository_inputVerifier, await InputVerifier_js_1.InputVerifier.create(ethersReadonlyProvider, __classPrivateFieldGet(repo, _FhevmContractsRepository_fhevmExecutor, "f").inputVerifierAddress, config.inputVerifierAbi), "f");
        __classPrivateFieldSet(repo, _FhevmContractsRepository_kmsVerifier, await KMSVerifier_js_1.KMSVerifier.create(ethersReadonlyProvider, config.kmsContractAddress, config.kmsVerifierAbi), "f");
        __classPrivateFieldSet(repo, _FhevmContractsRepository_hcuLimit, await HCULimit_js_1.HCULimit.create(ethersReadonlyProvider, __classPrivateFieldGet(repo, _FhevmContractsRepository_fhevmExecutor, "f").hcuLimitAddress, config.hcuLimitAbi), "f");
        if (config.zamaFheDecryptionOracleAddress !== undefined) {
            __classPrivateFieldSet(repo, _FhevmContractsRepository_zamaFheDecryptionOracle, await ZamaFheDecryptionOracle_js_1.ZamaFheDecryptionOracle.create(ethersReadonlyProvider, config.zamaFheDecryptionOracleAddress, config.zamaFheDecryptionOracleAbi), "f");
        }
        if (__classPrivateFieldGet(repo, _FhevmContractsRepository_inputVerifier, "f").gatewayChainId !== __classPrivateFieldGet(repo, _FhevmContractsRepository_kmsVerifier, "f").gatewayChainId) {
            throw new error_js_1.FhevmError(`gateway chainId mismatch. InputVerifier.gatewayChainId=${__classPrivateFieldGet(repo, _FhevmContractsRepository_inputVerifier, "f").gatewayChainId} differs from KMSVerifier.gatewayChainId=${__classPrivateFieldGet(repo, _FhevmContractsRepository_kmsVerifier, "f").gatewayChainId}`);
        }
    ...
    addressToContractMap() {
        (0, error_js_1.assertFhevm)(__classPrivateFieldGet(this, _FhevmContractsRepository_addressToContract, "f") !== undefined, "FhevmContractsRepository is not initialized");
    ...
    getContractFromName(name) {
        switch (name) {
            ...
            case "DecryptionOracle":
                return this.zamaFheDecryptionOracle;
            default: {
                throw new error_js_1.FhevmError(`Unsupported contract ${name}`);
            }
        }
    }
    getCoprocessorContractFromName(name) {
        switch (name) {
            ...
            case "HCULimit":
                return this.hcuLimit;
            default: {
                throw new error_js_1.FhevmError(`Unsupported coprocessor contract ${name}`);
            }
        }
    }
    getDecryptionOracleContractFromName(name) {
        switch (name) {
            case "DecryptionOracle":
                return this.zamaFheDecryptionOracle;
            default: {
                throw new error_js_1.FhevmError(`Unsupported decryption oracle contract ${name}`);
            }
        }
    }
    ...
    get acl() {
        (0, error_js_1.assertFhevm)(__classPrivateFieldGet(this, _FhevmContractsRepository_acl, "f") !== undefined, "FhevmContractsRepository is not initialized");
        return __classPrivateFieldGet(this, _FhevmContractsRepository_acl, "f");
    }
    get fhevmExecutor() {
        (0, error_js_1.assertFhevm)(__classPrivateFieldGet(this, _FhevmContractsRepository_fhevmExecutor, "f") !== undefined, "FhevmContractsRepository is not initialized");
        return __classPrivateFieldGet(this, _FhevmContractsRepository_fhevmExecutor, "f");
    }
    get inputVerifier() {
        (0, error_js_1.assertFhevm)(__classPrivateFieldGet(this, _FhevmContractsRepository_inputVerifier, "f") !== undefined, "FhevmContractsRepository is not initialized");
        return __classPrivateFieldGet(this, _FhevmContractsRepository_inputVerifier, "f");
    }
    get kmsVerifier() {
        (0, error_js_1.assertFhevm)(__classPrivateFieldGet(this, _FhevmContractsRepository_kmsVerifier, "f") !== undefined, "FhevmContractsRepository is not initialized");
        return __classPrivateFieldGet(this, _FhevmContractsRepository_kmsVerifier, "f");
    }
    get hcuLimit() {
        (0, error_js_1.assertFhevm)(__classPrivateFieldGet(this, _FhevmContractsRepository_hcuLimit, "f") !== undefined, "FhevmContractsRepository is not initialized");
...
exports.FhevmContractsRepository = FhevmContractsRepository;
